...
class Antenna(object):
    ...
    def __init__(self, id_a, config, options):
        ...
        self.fixed = True if "fixed" in config and config["fixed"] == 1 else False
    ...
    def propagate(self, x, slice=None):

        if slice is None:
            capacity = self.capacity
        else:
            capacity = self.capacity*slice["size"]

        loss = 0
        if x > capacity:
            loss = x-capacity
            x = capacity
        fh = self.computeFH(x, slice=slice)
        mh = self.computeMH(x, slice=slice)
        bh = self.computeBH(x)
    ...
    def computeFH(self, x, slice=None):
        if slice is None:
            f1 = 1
        else:
            f1 = slice["size"]
        if self.split_FH == 7.2:
            ...
            fh = (f1*load*self.NSC*self.NS*self.NL*self.W*self.C+self.MAC*(1e6))/float(1e9)
        elif self.split_FH == 2:
            fh = f1*self.PR*self.BW/self.BWR*self.NL/self.NLR*self.M/self.ML+self.S
        elif self.split_FH == 4:
            fh = f1*self.PR*self.BW/self.BWR*self.NL/self.NLR*self.M/self.ML
    ...
    def computeMH(self, x, slice=None):
        ...
        if self.split_MH == 2:
            mh = f1*self.PR*self.BW/self.BWR*self.NL/self.NLR*self.M/self.ML+self.S
        elif self.split_MH == 4:
            mh = f1*self.PR*self.BW/self.BWR*self.NL/self.NLR*self.M/self.ML
